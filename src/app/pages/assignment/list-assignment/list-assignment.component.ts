...
export class ListAssignmentComponent implements OnInit {
  ...
  paginatorEvent(page: number): void {
    this.assignmentService
      .getAssignmentList(
        page,
        this.assignmentService.assignmentsPaginationData.value.limit,
        this.searchValue
      )
      .subscribe((message) => {
        if (!isEmpty(message)) {
  ...
  search() {
    ...
    this.assignmentService
      .getAssignmentList(page, limit, this.searchValue)
      .subscribe((message) => {
        if (!isEmpty(message)) this.snackbarService.showAlert(String(message));
      });
  }
}
